...
//逃げ, 追いかけの回数
int nigeR, nigeB, oikakeR, oikakeB;
void AddNigeR(bool printLog = false) { nigeR++; if (printLog) std::cout << "Add nigeR" << std::endl; }
void AddNigeB(bool printLog = false) { nigeB++; if (printLog) std::cout << "Add nigeB" << std::endl; }
void AddOikakeR(bool printLog = false) { oikakeR++; if (printLog) std::cout << "Add oikakeR" << std::endl; }
void AddOikakeB(bool printLog = false) { oikakeB++; if (printLog) std::cout << "Add oikakeB" << std::endl; }
...
namespace red {
  ...
  void myTurn(char board[6][6]) {
    ...
    if (isOikake(block, mv)) {
      std::cerr << "Enemy(" << ny << ", " << nx << ") is Oikaked." << std::endl;
      if (prevMyRed == 1) {
        std::cerr << "相手はピンチだった" << std::endl;
        eval[histCnt - 1][ny][nx] += weightOikakePinti;
      }
      else {
        std::cerr << "相手は余裕だった" << std::endl;
...
std::pair<MoveCommand, int> thinkKanzen(int X) {
  ...
  if (cnt == 0) return std::pair<MoveCommand, int>(MoveCommand(-1, -1, -1), 0);
...
std::pair<MoveCommand, int> thinkMove() {
  std::pair<MoveCommand, int> resK, resP;
...
std::string solve(int turnCnt) {
  ...
  */
  ...
  Value evaluate(int teban, Position pos) {
    //int s0 = kbb::weight1 * kbb::bitCount(existB) - kbb::weight2 * kbb::myGoalDist(existB | existR);
    int s0 = kbb::weight1 * pos.count<BLUE>(WHITE) - kbb::weight2 * kbb::myGoalDist(existB | existR);
    //int s1 = kbb::weight1 * kbb::bitCount(existb) - kbb::weight2 * kbb::yourGoalDist(existb | existr);
    int s1 = kbb::weight1 * pos.count<BLUE>(BLACK) - kbb::weight2 * kbb::yourGoalDist(existb | existr);
